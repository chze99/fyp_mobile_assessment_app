...
class LecturerAssessmentAddSA extends StatefulWidget {
  @override
  int question_paper_id = 0;
  String mode = "";
  int question_paper_detail_id = 0;
  LecturerAssessmentAddSA(
      this.question_paper_id, this.mode, this.question_paper_detail_id);

  _LecturerAssessmentAddSAState createState() =>
      _LecturerAssessmentAddSAState();
}

class _LecturerAssessmentAddSAState extends State<LecturerAssessmentAddSA> {
  bool isLoading = true;
  bool isInitial = true;
  ...
  save_sa() async {
    ...
    var res = await Api().postData(data, "saveSAQuestion");
    var body = json.decode(res.body);
    if (body['success'] == true) {
      if (this.mounted) {
        return body['success'];
  ...
  update_sa() async {
    ...
    var res = await Api().postData(data, "updateSAQuestion");
    var body = json.decode(res.body);
    if (body['success'] == true) {
      if (this.mounted) {
        return body['success'];
